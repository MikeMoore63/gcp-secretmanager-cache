...
class NoActiveSecretVersion(SecretCacheError):
    ...
    def __init__(self, secret):
        super(NoActiveSecretVersion, self).__init__(
            self.CUSTOM_ERROR_MESSAGE.format(secret)
        )
...
class DBPWDInputUnsafe(SecretRotatorError):
    ...
    def __init__(self, secret_id, username):
        super(DBINputUnsafe, self).__init__(
            self.CUSTOM_ERROR_MESSAGE.format(secret_id, username)
        )
...
class NewSecretCreateError(SecretRotatorError):
    ...
    def __init__(self, secret_id, request_body, error):
        super(NewSecretCreateError, self).__init__(
            self.CUSTOM_ERROR_MESSAGE.format(secret_id, str(request_body), str(error))
        )
        self._error = error
